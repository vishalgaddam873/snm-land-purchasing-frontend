...
export function SectorsBulkExcelControls({
  onImported,
}: {
  onImported: () => void;
}) {
  ...
  async function downloadSample() {
    ...
    try {
      const res = await fetch("/api/sectors/import/sample", {
        credentials: "include",
      });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = "sector-import-sample.xlsx";
      a.click();
  ...
  async function onFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch("/api/sectors/import", {
        method: "POST",
        body: fd,
        credentials: "include",
      });
    ...
    <>
      ...
      <Dialog open={resultOpen} onOpenChange={setResultOpen}>
        <DialogContent className="max-h-[85vh] max-w-lg overflow-y-auto sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>Sector import finished</DialogTitle>
            <DialogDescription>
