...
export const EMPTY_USER_LIST_FILTERS: UserListFilterValues = {
  role: "",
  status: "",
};

const ENUM_FILTER_ALL = "__user_enum_all__";

export function countActiveUserFilters(f: UserListFilterValues): number {
...
type UsersFilterBarProps = {
  ...
  appliedFilters: UserListFilterValues;
  onApplyFilters: (next: UserListFilterValues) => void;
  onClearAll: () => void;
  className?: string;
};
...
export function UsersFilterBar({
  ...
  const [draftFilters, setDraftFilters] =
    React.useState<UserListFilterValues>(appliedFilters);

  const roleTriggerLabel = React.useMemo(
    () =>
      draftFilters.role
        ? labelFromSnake(draftFilters.role)
        : "All roles",
    [draftFilters.role],
  );
  ...
  const appliedFilterCount = countActiveUserFilters(appliedFilters);
  ...
  function resetDraft() {
    setDraftFilters({ ...EMPTY_USER_LIST_FILTERS });
  }

  function patchDraft(patch: Partial<UserListFilterValues>) {
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs text-muted-foreground">
          Search name, email, username, or contact. Use filters for role and
          status — then{" "}
        ...
        >
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FilterField label="Role" htmlFor="dlg-user-filter-role">
                <Select
                  value={draftFilters.role || ENUM_FILTER_ALL}
                  onValueChange={(v) =>
                    patchDraft({
                      role: v === ENUM_FILTER_ALL ? "" : (v ?? ""),
                    })
                  }
                >
                  <SelectTrigger
                    id="dlg-user-filter-role"
                  ...
                  <SelectContent>
                    <SelectItem value={ENUM_FILTER_ALL}>All roles</SelectItem>
              ...
              <FilterField label="Status" htmlFor="dlg-user-filter-status">
                <Select
                  value={draftFilters.status || ENUM_FILTER_ALL}
                  onValueChange={(v) =>
                    patchDraft({
                      status: v === ENUM_FILTER_ALL ? "" : (v ?? ""),
                    })
                  }
                >
                  <SelectTrigger
                    id="dlg-user-filter-status"
                    className={selectTriggerClass}
                  >
                  ...
                  <SelectContent>
                    <SelectItem value={ENUM_FILTER_ALL}>
                      All statuses
                    </SelectItem>
