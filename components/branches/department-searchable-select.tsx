...
function deptOptionId(d: DepartmentSelectOption): string {
  if (d._id != null && d._id !== "") {
    return String(d._id);
  }
  return "";
...
function deptLine(d: DepartmentSelectOption): string {
...
type DepartmentSearchableSelectProps = {
  id?: string;
  ...
  return (
    <MenuPrimitive.Root open={open} onOpenChange={setOpen}>
      <MenuPrimitive.Trigger
        ...
        disabled={disabled}
        type="button"
        data-slot="department-search-trigger"
        className={cn(
      ...
      <MenuPrimitive.Portal>
        ...
        >
          <MenuPrimitive.Popup
            data-slot="department-search-popup"
            className={cn(
          ...
          >
            ...
            <div className="max-h-52 min-h-0 overflow-y-auto p-1">
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((d) => {
                  const did = deptOptionId(d);
                  if (!did) return null;
                  const selected = value === did;
                  return (
                    <MenuPrimitive.Item
                      key={did}
                      className={menuItemClass}
                      onClick={() => commit(did)}
                    >
                      <span className="line-clamp-1 flex-1">{deptLine(d)}</span>
                      {selected ? (
                        <span className="pointer-events-none absolute right-2 flex size-4 items-center justify-center">
                          <CheckIcon className="size-4" />
                        </span>
